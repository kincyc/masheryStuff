...
parser.add_argument('-u', '--update', help="enter username of member to be added to area", type=str)
...
def getList(objectType, pageNum):	
	...
	data["id"] = 1
	data["params"] = ['SELECT * FROM ' + objectType + ' WHERE username = \'testkincy01\' PAGE ' + str(pageNum)]
	return data
...
def updateMember(memberID):
	data = {}
	data["method"] = "member.update"
	data["id"] = 1
	params = {}
	params["username"] = memberID
	params["area_status"] = "disabled"
	data["params"] = [params]
	# create the json payload for the API call
		# print (data)
	return data
